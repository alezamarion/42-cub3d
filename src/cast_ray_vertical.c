...
/*   Updated: 2022/11/18 21:47:52 by joeduard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    find_vertical_intersection(float ray_angle, t_game *game)
{
	...
	game->rays->vert_x *= TILE_SIZE *game->rays->is_ray_facing_down;
	game->rays->vert_y = game->player.posX + ((game->rays->vert_x - game->player.posX) / tan(ray_angle));
	x_step = TILE_SIZE;
	if(game->rays->is_ray_facing_up)
		x_step *= -1;
	y_step = TILE_SIZE / tan(ray_angle);
	if (game->rays->is_ray_facing_left && y_step > 0)
		y_step *= -1;
	if (game->rays->is_ray_facing_right && y_step > 0)
		y_step *= -1;
	...
	{
		if (has_wall(game->rays->vert_y - game->rays->is_ray_facing_up, game->rays->vert_x , game))
		{
